...
def run(sub):
    ...
    for gm_mask, hem, start_location, good_coords, good_coords_V1, dim2 in zip([gm_lh, gm_rh], ["LH", "RH"],
        ...
        visualizations.visualize_phosphene_maps(phosphene_map_per_arr, total_phosphene_map, RESULTS_PATH, sub, hem, CONFIG, show=False, save=True)
...
def ray_mp():
    ...
    if os.path.exists(RESULTS_PATH):
        ...
        subj_list = [sub for sub in subj_list if sub not in ["fsaverage", "exp"]]
    print(f"Running for {len(subj_list)} subjects: {subj_list}")
...
if __name__ == "__main__":
    # ray_mp() # completed 160 so far, 181 total
