...
# visual degrees - -15 is probably lowest possible angle,
# otherwise other hem is in the way if hem = RH -> low = -110, high = 15
DIM2 = Integer(name='beta', low=-15, high=110)
...
@ray.remote
def run(sub, target_name, target):
    global start_location, gm_mask, target_density, bin_thresh
    ...
    target_density = target.copy()
    print(f"Now at {target_name}")
...
def ray_mp():
    ...
    targets = (
    [gvs.upper_sector(windowsize=CONFIG["WINDOWSIZE"], fwhm=800, radiusLow=0, radiusHigh=500, plotting=False),
     ...
     gvs.complete_gauss(windowsize=CONFIG["WINDOWSIZE"], fwhm=1200, radiusLow=0, radiusHigh=500, center=None,
                        plotting=False)])
    ...
    futures = [run.remote(sub, target_name, target) for target_name, target in zip(target_names, targets)]
    ...
    for sub in all_results:
        ...
        if len(os.listdir(sub_dir)) == 2:
            ...
            for sub_hem in sub_hems:
                if len(os.listdir(sub_dir + sub_hem)) != 10:
...
if __name__ == "__main__":
    ...
    # check_all_files(RESULTS_PATH)

    # subj_list = ['exp', '114823', '200311', '146129', '125525', '205220']
